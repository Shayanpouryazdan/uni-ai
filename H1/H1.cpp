...
// محاسبه مقدار هیوریستیک (مجموع فاصله منهتن)
...
// الگوریتم A* برای حل پازل با هیوریستیک منهتن
void solvePuzzle(vector<vector<int>> start) {
    ...
    int h = calculateManhattanDistance(start);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nx >= 0 && nx < 3 && ny >= 0 && ny < 3) {
                ...
                if (visited.find(newStateStr) == visited.end()) {
                    ...
                    int newH = calculateManhattanDistance(newState);
